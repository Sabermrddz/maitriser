        ...
        <div key={current._id}>
          ...
          {isSubmitted && (
            <div className={`case-result-box ${results[current._id].correct ? 'correct' : 'incorrect'}`}>
              ...
              {results[current._id].optionExplanations && results[current._id].optionExplanations.length > 0 && (
                <div className="explanation-box" style={{ marginTop: 8 }}>
                  ...
                  {results[current._id].optionExplanations.map((expl) => (
              ...
              {results[current._id].keyConcepts && results[current._id].keyConcepts.length > 0 && (
                <div className="explanation-box" style={{ marginTop: 8 }}>
                  <strong style={{ display: 'block', marginBottom: 4 }}>{t('admin.quiz.keyConcepts', 'Concepts clés')}</strong>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {results[current._id].keyConcepts.map((c, i) => (
              ...
              {results[current._id].commonTraps && results[current._id].commonTraps.length > 0 && (
                <div className="explanation-box" style={{ marginTop: 8 }}>
                  <strong style={{ display: 'block', marginBottom: 4 }}>{t('admin.quiz.commonTraps', 'Pièges fréquents')}</strong>
                  <ul style={{ margin: 0, paddingLeft: 18 }}>
                    {results[current._id].commonTraps.map((trap, i) => (
